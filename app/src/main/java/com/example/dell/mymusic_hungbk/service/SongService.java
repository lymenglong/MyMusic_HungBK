...
import com.example.dell.mymusic_hungbk.playmusic.SlidePagePlayMusicFragment;
...
public class SongService extends Service implements AudioManager.OnAudioFocusChangeListener {
     ...
     * điều khiển luồng của thanh progress và thời gian
     *
     * @author jonty.ankit
     */
    ...
    private final Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (mp != null) {
                ...
                i[0] = mp.getCurrentPosition();// lấy vị trí hiện tại của thời gian phát nhạc
                i[1] = mp.getDuration();//lấy toàn bộ thời gian bài hát
                i[2] = progress;
                try {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            ...
            if (currentVersionSupportLockScreenControls) {
                RegisterRemoteClient();// đămg kí chức năng play nhạc
            }
            String songPath = data.getLink();//lấy đường link nhạc
            playSong(songPath, data);
            newNotification();

            PlayerConstants.SONG_CHANGE_HANDLER = new Handler(new Callback() {
                //điều khiển luồng của bài hát
                @Override
                public boolean handleMessage(Message msg) {
                    ...
                    try {
                        ...
                        SlidePagePlayMusicFragment.changeUI();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    return false;
            ...
            PlayerConstants.PLAY_PAUSE_HANDLER = new Handler(new Callback() {
                ...
                public boolean handleMessage(Message msg) {
                    ...
                    try {
                        ...
                        SlidePagePlayMusicFragment.changeButton();
                    } catch (Exception e) {
                    }
    ...
    //khi đầy bộ nhớ thì nó tự giải phóng
    @Override
    ...
    private void playSong(String songPath, SongBean data) {
        try {
            ...
            mp.reset();//bắt đầu phát nhạc từ đầu
            mp.setDataSource(songPath);// tập hợp các nguồn dữ liệu của tập tin âm thanh
            mp.prepare();//chuẩn bị để phát lại đồng bộ
            mp.start();// Bắt đầu hoặc tiếp tục phát lại
            timer.scheduleAtFixedRate(new MainTask(), 0, 100);//điều khiển thời gian
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    // điều khiển từng chức năng
    @SuppressLint("NewApi")
